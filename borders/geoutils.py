...
def get_raw_geometries(
    obj: shapely.geometry.base.BaseGeometry
) -> typing.List[shapely.geometry.base.BaseGeometry]:
    if not obj.is_empty and isinstance(
        obj, shapely.geometry.base.BaseMultipartGeometry
    ):
    ...
    if (
        ...
        return shapely.ops.linemerge(
            shapely.geometry.MultiLineString(
                [x for x in obj if not isinstance(x, shapely.geometry.Point)]
            )
        )
    return obj


def create_multi_string(
    obj1: shapely.geometry.base.BaseGeometry, obj2: shapely.geometry.base.BaseGeometry
) -> shapely.geometry.MultiLineString:
    geoms = []
...
def split_intersec(
    intersec: shapely.geometry.base.BaseGeometry,
    objs: typing.List[shapely.geometry.base.BaseGeometry],
) -> shapely.geometry.base.BaseGeometry:
    rv = intersec
    for obj in objs:
        ...
        for geom in obj.geoms:
            ...
            if geom.intersects(intersec) and not isinstance(
                small_intersec, (shapely.geometry.Point, shapely.geometry.MultiPoint)
            ):
                rest = get_raw_geometries(rv.difference(small_intersec))
                if rest:
                    rv = shapely.geometry.MultiLineString(
                        (*get_raw_geometries(small_intersec), *rest)
                    )
    return rv
...
def split_by_common_ways(borders: typing.List[Feature]) -> typing.List[Feature]:
    for (border, other) in itertools.combinations(borders, 2):
        __log.debug(
            "Processing border ({0}, {1})".format(
                borders.index(border), borders.index(other)
            )
        )
        ...
        border.geometry = create_multi_string(
            intersec, border.geometry.difference(intersec)
        )
        other.geometry = create_multi_string(
            intersec, other.geometry.difference(intersec)
        )
    return borders
