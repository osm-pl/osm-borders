...
class KmlShapelyTests(unittest.TestCase):
    def test1(self):
        with open("example.kml") as f:
            ...
            self.assertEqual(
                "layer_miejscowosci_granica.1488043", obj[0].get_tag("name")
            )
    ...
    def test_multi_geometry(self):
        ...
        for feature in features:
            self.assertTrue(
                isinstance(
                    feature.geometry, shapely.geometry.base.BaseMultipartGeometry
                ),
                "Geometry {0} is not multipart for Feature {1}".format(
                    type(feature.geometry), feature
                ),
            )
